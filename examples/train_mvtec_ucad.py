...
def train_task(model: UCADModel, category: str) -> None:
    train_samples = load_split(category, "train")[:MAX_TRAIN_SAMPLES]
...
def evaluate(model: UCADModel, category: str) -> tuple[float, float, float]:
    ...
    img_auroc = roc_auc_score(y_true_image, results.anomaly_scores)
    logger.info(f"{category} Image-level ROC-AUC: {img_auroc:.4f}")

    y_true_pixel = load_pixel_ground_truth(test_samples).reshape(-1)
    y_score_pixel = results.score_maps.reshape(-1)
    pixel_auroc = roc_auc_score(y_true_pixel, y_score_pixel)
    pixel_aupr = average_precision_score(y_true_pixel, y_score_pixel)
    logger.info(f"{category} Pixel-level ROC-AUC: {pixel_auroc:.4f}")
    logger.info(f"{category} Pixel-level AUPR: {pixel_aupr:.4f}")

    return img_auroc, pixel_auroc, pixel_aupr
...
def main():
    config = UCADConfig(
        ...
        reweighting_num_nn=REWEIGHTING_NUM_NN,
        patchsize=PATCHSIZE,
    )
    ...
    for category in CATEGORIES:
        train_task(model, category)

    for category in CATEGORIES:
        evaluate(model, category)
