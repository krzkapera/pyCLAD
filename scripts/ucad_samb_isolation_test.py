...
SAMB_ROOT = os.environ["SAMB_ROOT"]
CATEGORY = os.environ.get("ISOLATION_CATEGORY", "screw")
PATCHSIZE = int(os.environ.get("ISOLATION_PATCHSIZE", "3"))
INPUT_SIZE = (224, 224)
SEED = 0
...
def main():
    ...
    config = UCADConfig(max_tasks=1, patchsize=PATCHSIZE, sam_masks_dir=SAMB_ROOT, sam_images_root=MVTEC_ROOT)
    model = UCADModel(config)

    best = train_task_with_authors_tracking(model, CATEGORY)
    logger.info(
        f"RESULT protocol=authors masks=sam-b patchsize={PATCHSIZE} category={CATEGORY} image_auroc={best['image_auroc']:.4f} "
    ...
    logger.info(
        f"RESULT protocol=ours masks=sam-b patchsize={PATCHSIZE} category={CATEGORY} image_auroc={image_auroc:.4f} "
        f"pixel_auroc={pixel_auroc:.4f} pixel_aupr={pixel_aupr:.4f}"
    )
